...
// .wrangler/tmp/bundle-L03tCf/checked-fetch.js
...
// .wrangler/tmp/bundle-L03tCf/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-L03tCf/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-L03tCf/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
async function getServiceStatus(env) {
  const now = /* @__PURE__ */ new Date();
  const formatDate = /* @__PURE__ */ __name((date) => date.toLocaleString("en-US", {
    month: "short",
    day: "numeric",
    hour: "2-digit",
    minute: "2-digit",
  ...
  let twitchStatus = { status: "offline", lastTested: formatDate(now), error: "Test failed" };
  try {
  ...
  let githubStatus = { status: "offline", lastTested: formatDate(now), error: "Test failed" };
  try {
  ...
  let aiStatus = { status: "offline", lastTested: formatDate(now), error: "Not available" };
  try {
    if (env.ai) {
  ...
  let storageStatus = { status: "offline", lastTested: formatDate(now), error: "Not available" };
  try {
    if (env.R2_BUCKET) {
